...
| 2026-08-02 | upstream sync | **Merged 131 upstream commits** (`18484f23a`..`f0aa3a8bb`, through browser-use 0.13.7) into `franky-rust` — zero conflicts; this fork's Python footprint is only AGENTS.md/CLAUDE.md/`contrib/mcp/mcp-launch.py`. All 14 new upstream CI tests pass (the one apparent failure is environmental: a SOCKS proxy env var without `socksio`). | **done** |
| 2026-08-02 | upstream→rust port | **7 defects fixed, 6 with executed tests.** A 19-agent map+adversarial-verify pass over the 131 commits confirmed 7 of 12 candidates and refuted 5. Ported: (1) **iframe clicks hit the wrong element** — DOMSnapshot bounds are frame-LOCAL, so a click inside an offset iframe fired a *different* top-document button; now resolved via `DOM.getContentQuads` (+ a forced reflow, because quads are stale right after a DOM mutation and `getDocument` does not fix it); (2) paint-order occlusion scoped per document (upstream `211091828`) — an opaque top-document header was culling *every* element in every iframe; (3) React controlled inputs never cleared (upstream `4ebc0f5d0`) — instance setter → prototype native setter; (4) `finish_reason="length"` truncation undetected (upstream `5f690c207`); (5) a stalled/failed state screenshot sank the whole snapshot (upstream `7f22c5a7b`); (6) readiness timeout failed the navigation instead of reporting a partial load (upstream `_navigate_and_wait`) — was a 30s hard error, now succeeds in 9s with a usable DOM; (7) agent aborted the run on one failed capture (upstream `e7cc40f5a`). Also added `browser_set_viewport` (17th tool, fork-only). 66 tests green; clippy `-D warnings` clean. | **done** |
