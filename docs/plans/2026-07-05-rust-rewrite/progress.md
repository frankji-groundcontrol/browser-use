...
| 2026-07-05 | 1c | First-cut selector-map + `browser_click`/`browser_type` (functional, not yet Python-quality). | in progress |
| — | 1d | Full-quality `bu-dom` 5-stage serializer for Python parity + golden `tools/list`/`initialize` diff + live conformance. **Only then shadow the `browser-use` name.** | pending |
